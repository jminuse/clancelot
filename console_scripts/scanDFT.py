...
# Plot energies
def plot(yy, start_val, x_label, y_label, title, x_range, y_range, x_low=0):
...
# DEFAULTS ARE HERE
dft, u1, u2, scale, step, out_name, comp, neb_force = 'orca', 'Ha', 'kT_300', 1.0, 1, 'out', None, None
...
if len(sys.argv) < 3:
	dft = raw_input("What method of dft was used (orca/g09, default orca)? ").lower().strip()
	if dft == 'g09':
		directory = "gaussian"
		read = g09.read
	...
	if dft == 'g09':
		...
		while os.path.isfile("%s/%s-%d-%d/%s-%d-%d.out" % (directory, run_name, N, M, run_name, N, M)):
			M += 1
		max_frame = M - 1
		while os.path.isfile("%s/%s-%d-1/%s-%d-1.out" % (directory, run_name, N, run_name, N)):
			N += 1
		max_iter = N - 1
		# Verify the last iteration did indeed succeed
		success = True
	...
	if plot_all in ['y','yes','sure','ok','do it','i dare you']:
		...
		try:
			iterations_to_plot = input("\nWhich iterations would you like to plot? Input as a python range (ex. range(3,6) for iterations 3,4,5): ")
		except:
	...
	if plotting_flags != "": u2 = plotting_flags

	plotting_flags = raw_input("\nScale (%lg): " % scale).strip()
	if plotting_flags != "": scale = float(plotting_flags)

	plotting_flags = raw_input("\nPlot Title (%s): " % title).strip()
	if plotting_flags != "": title = plotting_flags
	
	try:
		plotting_flags = raw_input("\nX Axis Title: ").strip()
		if plotting_flags != "": x_label = plotting_flags
	except:
		print("\tDefaulting, X Axis label is \"%s\"...\n" % x_label)

	try:
		plotting_flags = raw_input("\nY Axis Title: ").strip()
		if plotting_flags != "": y_label = plotting_flags
	except:
